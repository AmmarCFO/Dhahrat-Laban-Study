...
import CompSetModal from './components/CompSetModal';
...
const App_en: React.FC<{ onToggleLanguage: () => void }> = ({ onToggleLanguage }) => {
  ...
  const [mabaatPercentage, setMabaatPercentage] = useState<number>(0.15); // Default to 15%
      ...
      { 
          ...
          label: (
              <div className="flex flex-col items-center justify-center gap-0.5">
                  <span className="leading-none text-xs sm:text-sm">SAR 2,900<span className="text-[0.8em] opacity-70 font-normal ml-0.5">/mo</span></span>
                  <span className="text-[0.65em] opacity-50 font-medium leading-none">SAR 34,800 / yr</span>
              </div>
          ) 
      },
      { 
          ...
          label: (
              <div className="flex flex-col items-center justify-center gap-0.5">
                  <span className="leading-none text-xs sm:text-sm">SAR 3,200<span className="text-[0.8em] opacity-70 font-normal ml-0.5">/mo</span></span>
                  <span className="text-[0.65em] opacity-50 font-medium leading-none">SAR 38,400 / yr</span>
              </div>
          )
      },
      { 
          ...
          label: (
              <div className="flex flex-col items-center justify-center gap-0.5">
                  <span className="leading-none text-xs sm:text-sm">SAR 3,500<span className="text-[0.8em] opacity-70 font-normal ml-0.5">/mo</span></span>
                  <span className="text-[0.65em] opacity-50 font-medium leading-none">SAR 42,000 / yr</span>
              </div>
          )
      },
  ];
  ...
  ledgerItems.push({ category: `Mathwaa Share (${Math.round(mabaatPercentage * 100)}%)`, amount: effectiveMabaat, color: 'bg-purple-400' });
        ...
        <FadeInUp>
          <div className="text-center pt-10 pb-8 sm:pt-16 sm:pb-8">
            ...
            <h1 className="text-4xl sm:text-6xl font-black text-[#1D1D1F] tracking-tighter mb-4 leading-none">
              Dhahrat Laban<span className="text-[#8A6E99]">.</span>
            </h1>
            <p className="text-base sm:text-xl text-gray-500 max-w-2xl mx-auto font-medium leading-relaxed tracking-tight px-4">
                Evaluation for the <span className="text-[#8A6E99]">17-Unit Residential Complex</span>. Focusing on <span className="text-[#2A5B64]">Long Term Rental (LTR) Model</span>.
            </p>
          </div>
        </FadeInUp>
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        ...
                        <div className="lg:col-span-7 space-y-6">
                            ...
                            {/* Unit Mix */}
                            <div className="bg-white/5 rounded-2xl p-5 sm:p-6 border border-white/10">
                                <h4 className="text-sm font-bold mb-4 text-white/90">17-Unit Inventory (1 Bedroom)</h4>
                                <div className="space-y-4">
                                            ...
                                            {unit.priceRange && (
                                                <div className="grid grid-cols-3 gap-2 bg-white/5 rounded-lg p-2.5">
                                                    <div className="text-center border-r border-white/10">
                                                        <p className="text-[9px] text-white/40 uppercase tracking-widest mb-0.5">Low</p>
                                                        <p className="text-xs font-bold text-white tabular-nums">{formatCurrency(unit.priceRange.min)}</p>
                                                    </div>
                                                    <div className="text-center border-r border-white/10">
                                                        <p className="text-[9px] text-emerald-400/60 uppercase tracking-widest mb-0.5 font-bold">Base</p>
                                                        <p className="text-sm font-black text-emerald-400 tabular-nums">{formatCurrency(unit.priceRange.avg)}</p>
                                                    </div>
                                                    <div className="text-center">
                                                        <p className="text-[9px] text-white/40 uppercase tracking-widest mb-0.5">High</p>
